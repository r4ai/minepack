...
pub async fn run(query: &str) -> Result<()> {
    ...
    let mods = client.search_mods(query, Some(&config.minecraft_version)).await
    ...
    for mod_info in mods.iter().take(10) {
        println!("{:<8} {:<40} {:<15} {}", 
            mod_info.id, 
            if mod_info.name.len() > 38 { format!("{}...", &mod_info.name[0..35]) } else { mod_info.name.clone() }, 
            format!("{}K", mod_info.download_count / 1000),
            if mod_info.summary.len() > 50 { format!("{}...", &mod_info.summary[0..47]) } else { mod_info.summary.clone() });
    }
