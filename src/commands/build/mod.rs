...
pub async fn run() -> Result<()> {
    ...
    println!("Output: {}/{}-{}.zip", build_dir.display(), config.name, config.version);
...
async fn build_multimc_pack(
    ...
    for mod_entry in &config.mods {
        let source_path = Path::new("mods").join(format!("{}-{}.jar", mod_entry.name, mod_entry.version));
    ...
    zip_directory(&temp_dir, &output_path)
...
async fn build_curseforge_pack(
    ...
    manifest.push_str(&format!("  \"minecraft\": {{\n"));
    manifest.push_str(&format!("    \"version\": \"{}\",\n", config.minecraft_version));
    manifest.push_str(&format!("    \"modLoaders\": [\n"));
    manifest.push_str(&format!("      {{\n"));
    manifest.push_str(&format!("        \"id\": \"{}-latest\",\n", loader_type));
    manifest.push_str(&format!("        \"primary\": true\n"));
    manifest.push_str(&format!("      }}\n"));
    manifest.push_str(&format!("    ]\n"));
    manifest.push_str(&format!("  }},\n"));
    manifest.push_str(&format!("  \"manifestType\": \"minecraftModpack\",\n"));
    manifest.push_str(&format!("  \"manifestVersion\": 1,\n"));
...
async fn build_modrinth_pack(
    ...
    index.push_str(&format!("  \"formatVersion\": 1,\n"));
    index.push_str(&format!("  \"game\": \"minecraft\",\n"));
    ...
    for (i, mod_entry) in config.mods.iter().enumerate() {
        ...
        index.push_str(&format!("      \"path\": \"mods/{}-{}.jar\",\n", mod_entry.name.replace(" ", "-"), mod_entry.version));
        index.push_str(&format!("      \"hashes\": {{}},\n"));
        index.push_str(&format!("      \"downloads\": [\"{}\"],\n", mod_entry.download_url));
        index.push_str(&format!("      \"fileSize\": 0\n"));
        index.push_str(&format!("    }}{}\n", if i < config.mods.len() - 1 { "," } else { "" }));
        pb.inc(1);
    }
    
    index.push_str("  ],\n");
    index.push_str(&format!("  \"dependencies\": {{\n"));
    index.push_str(&format!("    \"minecraft\": \"{}\",\n", config.minecraft_version));
    index.push_str(&format!("    \"{}\": \"*\"\n", loader_type));
    index.push_str("  }\n");
    index.push_str("}\n");
    ...
    zip_directory(&temp_dir, &output_path)
        .context("Failed to create mrpack archive")?;
...
fn zip_directory(src_dir: &Path, dst_file: &Path) -> Result<()> {
        ...
        for entry in fs::read_dir(src_dir)
            ...
            if path.is_file() {
                ...
                std::io::copy(&mut f, zip)
                    .context("Failed to copy file data to zip archive")?;
            } else if path.is_dir() {
    ...
    zip.finish().context("Failed to finish writing zip archive")?;
